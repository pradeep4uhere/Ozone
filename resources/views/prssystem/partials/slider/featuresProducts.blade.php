        ...
        <div class="product-description">
          ...
          <h1 class="product-description__title">
            <a href="{{route('details',['slug'=>str_slug($prodObj['Product']['title']),'id'=>encrypt($prodObj->id)])}}">            
              <?php if($prodObj['Product']['title']!=''){ ?>{{ str_limit(ucwords($prodObj['Product']['title']), $limit = 45, $end = '...') }}<?php }else{ echo "Unknown Name"; } ?>
              </a>
          </h1>

          <!-- category and price -->
          <div class="row">
            <div class="col-sm-7 product-description__category secondary-text">
          ...
          <div class="sizes-wrapper">
            ...
            Discount:{{($prodObj['discount_value']!=0)?$prodObj['discount_value']:'0'}}%
            </span>&nbsp;|&nbsp;
            <span class="secondary-text text-uppercase">
              Unit:{{$prodObj['quantity_in_unit']}}&nbsp;{{$prodObj['Product']['Unit']['name']}}
            </span>
          </div>
